...
class Runner extends ProviderAware {
  constructor(options = {}) {
    ...
    this.networkInfo = this.truffleConfig.networks[this.truffleConfig.network];
    this.commandResultTransport = new CommandResultTransport({
      level: 'debug',
      network: this.networkInfo.network_id,
  ...
  async read(globPathsAndScripts, state = {}) {
    ...
    if (this.options.dump) {
      ...
      const ext = path.extname(this.options.dumpFilename);
      let serialized;
      if ('.yml' === ext || '.yaml' === ext) {
        serialized = yaml.safeDump(output);
      } else if ('.json' === ext) {
        serialized = JSON.stringify(output, null, 2);
      } else if ('.js' === ext) {
        serialized = `module.exports = ${JSON.stringify(output, null, 2)};`;
      } else {
        throw new Error(`Unknown file type for dump: ${ext}`);
      }
  ...
  async run(command, state) {
    ...
    try {
      ...
      if (0 === command.run.indexOf('util.')) {
        ...
        //   const web3Opts = sortedInputs[sortedInputs.length - 1];
        //   const { from } = web3Opts;
        //   const balance = await web3.eth.getBalance(from);
        //   this.spinner.info(`\tVerified balance of from address: ${balance}`);
        // }
  ...
  getContract(contractName, state = {}) {
    ...
    Contract.setNetwork(this.networkInfo.network_id);
  ...
  async initState(state = {}) {
    ...
    // load already $deployed contracts into memory so they can be linked
    const $deployedKeys = Object.keys(state.$deployed);
    await Promise.each($deployedKeys, async (contractName) => {
      if (state.$contracts[contractName]) {
        return;
      }
      const Contract = this.getContract(contractName, state);
      await Contract.at(state.$deployed[contractName]);
      Contract.address = state.$deployed[contractName];
    });
  }
